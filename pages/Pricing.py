...
plans = {
    ...
    "Premium": {
        ...
        "features": [
            _lang["Unlimited use"],
            _lang["Premium"],
            _lang["AI doctor consultation"],
            _lang["... many more features"],
        ],
    },
}
...
with col1:
    ...
    features = "<br>".join(
        [f"{feature}" for feature in plans[plan_names[0]]["features"]]
    )
...
with col2:
    ...
    features = "<br>".join(
        [f"{feature}" for feature in plans[plan_names[1]]["features"]]
    )
    st.markdown(features, unsafe_allow_html=True)
    add_auth(
        required=False,  # Don't stop the app for non-subscribers
