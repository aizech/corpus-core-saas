...
# st.write(logo_image)
# breakpoint()
st.logo(logo_image, size="large", icon_image=icon_image)
...
if st.user.is_logged_in:
    with st.sidebar:
        ...
        if not is_subscribed:
            ...
            with col1p:
                st.markdown(
                    f"<p align='center' style='font-size: 20px; font-weight: bold;'>{_lang['Upgrade to Premium']}</p>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<p align='center' style='font-size: 14px;'>{_lang['Get 14 days free and unlock all Premium features.']}</p>",
                    unsafe_allow_html=True,
                )
        ...
        with col1s:
            if st.user.is_logged_in:
                ...
                st.markdown(
                    f"<a href='{account_page}' style='text-decoration: none;' target='_self'><div style='font-size: 24px; margin-top: 15px;'><img src='{avatar}' style='width: 34px; height: 34px; border-radius: 50%;'></div></a>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f"<a href='{account_page}' style='text-decoration: none;' target='_self'><div style='font-size: 24px; margin-top: 15px;'>:material/account_circle:</div></a>",
                    unsafe_allow_html=True,
                )
        with col2s:
            # is_subscribed = is_email_subscribed_to_product(st.user.email)
            ...
            st.markdown(
                f"<a href='{account_page}' style='text-decoration: none;' target='_self'><div style='margin-top: 15px;'>{st.user.name}</div></a>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<div style='margin-top: -22px; color:{color}; font-size: 12px'>{account_type}</div>",
                unsafe_allow_html=True,
            )

        if st.button(
            f":material/logout: {_lang['Sign out']}", width="stretch", type="secondary"
        ):
            st.logout()
            st.rerun()
        ...
        with col1t:
            ...
            if "theme" not in st.session_state or not isinstance(
        ...
        with col2t:
            ...
            try:
                current_index = (
                    available_languages.index(current_lang)
